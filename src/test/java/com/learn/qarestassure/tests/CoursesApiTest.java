...
public class CoursesApiTest {
	...
	public void verifyCoursesData(Map<String, String> provider) {
		...
		System.out.println("Purchase amount: " + Utilities.rawToJson(provider.get("body")).getInt("dashboard.purchaseAmount"));
		...
		List<Map> copies = Utilities.rawToJson(provider.get("body")).param("name", "RPA").get("courses.findAll {courses -> courses.title == name}");
		...
		Integer sumPrices = cursos.stream().map(m -> (Integer)m.get("copies") * (Integer)m.get("price")).reduce(0,Integer::sum);
